...
locals {
  lb_instances = {
    ...
    server_name =>
    {
      name       = vm_info.instances_details[0].name,
      id         = vm_info.instances_details[0].id,
      zone       = vm_info.instances_details[0].zone,
      ip_address = vm_info.instances_details[0].network_interface[0].network_ip
    }
    if local.servers[server_name].labels.application == "dynatrace"
  }
}
